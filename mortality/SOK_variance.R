...
SOK_full <- function(t,y,p){
  ...
  with(as.list(p), {
    ...
    dS.dt = -beta * S * V + lambda * R - d * S
    dR.dt = (1-m) * beta * sum(f * lagS * lagV) - lambda * R - d * R
    dV.dt = m * beta * sum(exp(-d * tau_min:tau_max) * f * ns * lagS * lagV) - delta * V
    dK.dt = m * beta * sum(exp(-d * tau_min:tau_max) * f * lagS * lagV)
    return(list(c(dS.dt, dR.dt, dV.dt, dK.dt)))
  })
}
...
#f = rep(0,K)
#f[17-tau_min+1] = 1
f = dnorm(tau_min:tau_max, 17, 1) / exp(-d * tau_min:tau_max)
f = f / sum(f)
p = list(beta=beta,ns=ns,f=f,delta=delta,d=d,m=m,lambda=lambda,offset=0)
#y0 = c(S0, rep(0,K), 0, f*P0)
out1 = dede(y=y0, times=ts, func=SOK_full, parms=p)


f = dnorm(tau_min:tau_max, 17,8) / exp(-d * tau_min:tau_max)
f = f / sum(f)
p = list(beta=beta,ns=ns,f=f,delta=delta,d=d,m=m,lambda=lambda,offset=0)
#y0 = c(S0, rep(0,K), 0, f*P0)
out2 = dede(y=y0, times=ts, func=SOK_full, parms=p)


data = data.frame(t = c(ts,ts),
                  virus = rep(c("Low","High"), each=length(ts)),
                  y = log10(c(out1[,4], out2[,4])))
ggplot(data) +
  aes(x=t, y=y, color=virus) +
  geom_line() +
  theme_classic() +
  xlab("Time since first infection (days)") +
  ylab("log10(Virus particles)") +
  ...
  labs(color = "Variability in SOK") + 
  scale_color_manual(values = c("red","black"))

plot(ts, out2[,2],type="l")
lines(ts, S0*exp(-d*ts),col="red")

plot(ts, out1[,5]/S0,type="l",col="red")
lines(ts, out2[,5]/S0)
...
#ns <- 10^(8.75 + (1/seq(1/.014,1/.006,(1/.006-1/.014)/(tau_max-tau_min))-.006) / (.014-.006) * (8.25-8.75))
#ns <- 1:K/(1:K+1)
ns = ns / mean(ns) * 1e6
...
ggplot(data) +
  ...
  scale_y_continuous(name="Virus particles in environment", labels=function(l) parse(text=paste0("10^",l)),
                     limits=if (concavity==0) c(8,11.75) else c(8,11), expand=expansion(c(.02,.02))) +
